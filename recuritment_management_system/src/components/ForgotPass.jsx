...
const ForgotPass = ({ role }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let apiUrl = "http://localhost:3000/resetPassword";
      
      if (role === "Candidate") {
        apiUrl = "http://localhost:3001/api/v1/candidates/reset-password";
      }
  ...
  return (
    <div className="flex justify-center items-center min-h-screen px-4 bg-slate-900 relative overflow-hidden">
      ...
      <div className="glass-card p-8 rounded-2xl shadow-2xl w-full max-w-md relative z-10 border border-slate-700/50">
        ...
        {message && (
          <p className="text-center mt-6 text-sm font-medium text-indigo-400 bg-indigo-500/10 py-2 rounded-lg border border-indigo-500/20">
            {message}
          </p>
        )}
      </div>
